...
```{r}
taxi_data$hourstart <- hour(taxi_data$pickup_datetime)#get hours as its own column 
length(unique(taxi_data$hourstart)) #check to see if there are 24 unique hours! 
```

We added an hourstart column to the taxi data set, by taking the hour value of pickup_datetime for each observation of taxi_data. We then checked that there were only 24 unique values representing the 24 hours. 

```{r}
options(scipen = 999)
Combinations<-crossing(taxi_data$hack_license,taxi_data$hourstart)
names(Combinations) <- c("hack_license", "hourstart")
taxi_data <- right_join(taxi_data,Combinations)
```

```{r}
# Test <- taxi_data[taxi_data$hack_license== 2013000001,]
# Test %>%
# group_by(hack_license,hourstart) %>%
# mutate(passenger_count = ifelse(is.na(passenger_count), 0, passenger_count), trip = ifelse(!is.na(medallion), TRUE, FALSE)) %>%
# summarise(passenger_count = sum(passenger_count), trips_started = sum(trip))
